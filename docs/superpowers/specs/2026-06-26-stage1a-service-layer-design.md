...
**Single-course CLI (behavior-preserving):** `planPush` and `executePush` run in **one locked session with no gap**. The lock already prevents vocgit-vs-vocgit interleaving exactly as today; there is **no execute-time re-fetch and no added API call**, so the API-call sequence is identical (satisfying the §8 golden test). For experimental `--sync-deletes`, the CLI intent records the confirmed reconciliation policy and deliberately defers its one remote file-list call until the historical post-upload position; detached org plans instead resolve exact existing-resource delete paths before confirmation and execute those paths without relisting. Cross-workspace concurrency was already an unguarded race *today* (the lock is local/advisory) — Stage 1a does not change that, for better or worse. No new `StalePlanError` path is exercised by the CLI.
