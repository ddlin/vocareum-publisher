...
> **Best-Effort Settings Synchronization:** Assignment and part settings in `vocareum.yaml` are synchronized on a best-effort basis. Vocareum does not advertise all possible settings in their API documentation, and we cannot validate settings values via the API before applying them. This means:
>
> 1. **Unknown Settings:** Some settings may exist in Vocareum that are not documented here. These will be preserved when importing but may not be fully understood.
> 2. **Validation Limitations:** Invalid setting values may be silently ignored by Vocareum or cause unexpected behavior. Always verify settings in the Vocareum UI after pushing.
> 3. **API Changes:** Vocareum may add, remove, or rename settings without notice. If a push fails due to settings, try removing the problematic setting and configuring it manually in the Vocareum UI.
> 4. **Type Coercion:** Some settings that appear numeric may need to be strings (e.g., `session_length: '60'` not `session_length: 60`). When in doubt, use string format.
...
async function createNewAssignment(
  ...
  logger.info('2. Run: vocgit validate');
  logger.info('3. Run: vocgit push (creates in Vocareum)');
...
async function fixValidationIssues(
  ...
  logger.info('Run vocgit validate to verify.');
}
```
...
- v1.1 (2026-02-15): Fixed section ordering; added best-effort settings documentation
